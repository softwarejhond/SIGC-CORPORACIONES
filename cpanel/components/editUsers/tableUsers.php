...
// Función para obtener sedes de un asesor
function getSedesAsesor($username, $conn)
{
    $sql = "SELECT sede1, sede2 FROM asesores_sedes WHERE username = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $username); // Cambiado a "s" ya que username es string
    $stmt->execute();
    $sede1 = null;
    $sede2 = null;
    $stmt->bind_result($sede1, $sede2);
    if ($stmt->fetch()) {
        $stmt->close();
        return $sede1 . (!empty($sede2) ? ', ' . $sede2 : '');
    }
    $stmt->close();
    return 'Sin sedes asignadas';
}

// Función para obtener opciones de sedes con selected
function getSedesOptions($conn, $selectedValue = '')
{
    $sql = "SELECT DISTINCT nombre FROM sedes ORDER BY nombre";
    $result = $conn->query($sql);
    $options = '<option value="">Seleccionar sede</option>';
    while ($row = $result->fetch_assoc()) {
        $selected = ($row['nombre'] == $selectedValue) ? 'selected' : '';
        $options .= '<option value="' . htmlspecialchars($row['nombre']) . '" ' . $selected . '>' . htmlspecialchars($row['nombre']) . '</option>';
    }
    return $options;
}
            ...
            <tbody>
                <?php foreach ($users as $user) { ?>
                    <tr>
                        ...
                        <td><?php echo ($user['rol'] == 3) ? getSedesAsesor($user['username'], $conn) : 'Rol sin sedes'; ?></td>
...
<!-- Modales de Edición -->
<?php foreach ($users as $user) {
    // Obtener sedes actuales para el asesor
    $sede1 = '';
    $sede2 = '';
    if ($user['rol'] == 3) {
        $sqlSedes = "SELECT sede1, sede2 FROM asesores_sedes WHERE username = ?";
        $stmtSedes = $conn->prepare($sqlSedes);
        $stmtSedes->bind_param("s", $user['username']); // Cambiado a "s"
        $stmtSedes->execute();
        $stmtSedes->bind_result($sede1, $sede2);
        $stmtSedes->fetch();
        $stmtSedes->close();
    }
?>
                ...
                <div class="modal-body">
                    <form id="updateForm<?php echo $user['id']; ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="rol" id="rolSelect<?php echo $user['id']; ?>" required onchange="toggleSedesFields(<?php echo $user['id']; ?>)">
                        ...
                        <!-- Campos de sedes para Asesor -->
                        <div id="sedesFields<?php echo $user['id']; ?>" style="display: <?php echo ($user['rol'] == 3) ? 'block' : 'none'; ?>;">
                            <div class="mb-3">
                                <label class="form-label">Sede 1</label>
                                <select class="form-select" name="sede1" id="sede1_<?php echo $user['id']; ?>">
                                    <?php echo getSedesOptions($conn, $sede1); ?>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Sede 2</label>
                                <select class="form-select" name="sede2" id="sede2_<?php echo $user['id']; ?>">
                                    <?php echo getSedesOptions($conn, $sede2); ?>
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    // Función para mostrar/ocultar campos de sedes
    function toggleSedesFields(userId) {
        const rolSelect = document.getElementById('rolSelect' + userId);
        const sedesFields = document.getElementById('sedesFields' + userId);
        sedesFields.style.display = (rolSelect.value == '3') ? 'block' : 'none';
    }
